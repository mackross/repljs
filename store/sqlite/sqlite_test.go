...
// newTestStore opens an in-memory SQLite store for test isolation.
// Each call creates an independent database.
...
// TestSQLiteStore_AppendFactRoundTrip is the primary round-trip test that the
// slice verification gate runs.  It appends a representative set of facts for
// a hand-crafted session and then exercises every query method to confirm data
// survives the SQLite encode/decode cycle without loss.
func TestSQLiteStore_AppendFactRoundTrip(t *testing.T) {
	...
	// Append session lifecycle facts in chronological order.
	facts := []model.Fact{
	...
	t.Run("LoadStaticEnv", func(t *testing.T) {
		...
		if len(env.CommittedSources) != len(wantSources) {
			t.Errorf("CommittedSources len: got %d, want %d", len(env.CommittedSources), len(wantSources))
		} else {
	...
	t.Run("LoadReplayPlan", func(t *testing.T) {
		...
		// Verify the replay decision for the effect carries the correct policy.
...
// TestSQLiteStore_FileBasedDSN confirms that the store works against a real
// on-disk file (not just in-memory), exercising WAL mode and busy timeout.
func TestSQLiteStore_FileBasedDSN(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	dsn := filepath.Join(dir, "test.db")

	s, err := sqlitestore.Open(ctx, dsn)
	if err != nil {
		t.Fatalf("Open file DSN: %v", err)
	}
	defer s.Close()

	fact := model.SessionStarted{
		Session:    "sess-file",
		RootBranch: "branch-main",
		At:         time.Now().UTC(),
	}
...
// TestSQLiteStore_NullableColumns confirms that facts appended without branch
// or cell identifiers persist with empty strings and do not break queries.
func TestSQLiteStore_NullableColumns(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)

	// ManifestAttached has no branch or cell — both nullable index columns
	// should be empty strings (our sentinel for NULL) and not cause insert errors.
	fact := model.ManifestAttached{
		Session:    "sess-nullable",
		ManifestID: "m1",
		Manifest: model.Manifest{
			ID:        "m1",
			Functions: nil,
	...
	if err := s.AppendFact(ctx, fact); err != nil {
		t.Fatalf("AppendFact ManifestAttached (no branch/cell): %v", err)
	}
}

func TestSQLiteStore_LoadReplayPlan_ExcludesParentRuntimeTransitionsAddedAfterFork(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
...
// TestSQLiteStore_BadDSN confirms that opening an invalid path returns an
// error instead of a partially initialised store.
func TestSQLiteStore_BadDSN(t *testing.T) {
	// A path into a directory that does not exist cannot be created by SQLite.
	_, err := sqlitestore.Open(context.Background(), "/nonexistent/dir/that/cannot/exist/test.db")
	if err == nil {
		t.Fatal("expected error opening bad DSN, got nil")
	}
}

// TestSQLiteStore_ManifestSnapshotPreserved checks that the full manifest
// snapshot (including function specs with nested json.RawMessage fields)
// round-trips correctly through SQLite.
func TestSQLiteStore_ManifestSnapshotPreserved(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	...
	if err := s.AppendFact(ctx, model.ManifestAttached{
		Session:    "sess-manifest",
		ManifestID: manifest.ID,
		Manifest:   manifest,
		At:         time.Now().UTC(),
	}); err != nil {
		t.Fatalf("AppendFact ManifestAttached: %v", err)
	}

	env, err := s.LoadStaticEnv(ctx, "sess-manifest", "unused-branch", "unused-cell")
	if err != nil {
		t.Fatalf("LoadStaticEnv: %v", err)
	}
	if env.Manifest.ID != manifest.ID {
...
// ---------------------------------------------------------------------------
// Focused LoadHead tests
// ---------------------------------------------------------------------------

// TestSQLiteStore_LoadHead covers the branch-existence and head-advance
// semantics defined in the store.Store contract.
func TestSQLiteStore_LoadHead(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("branch exists but no commits yet returns empty head", func(t *testing.T) {
		s := newTestStore(t)
		// SessionStarted records the root branch without any HeadMoved facts.
	...
	t.Run("forked branch returns its own head", func(t *testing.T) {
		s := newTestStore(t)
		// Root: cell1 → cell2.  Fork from cell1 → cell3.
		for _, f := range []model.Fact{
			model.SessionStarted{Session: session, RootBranch: root, At: time.Now().UTC()},
			model.ManifestAttached{Session: session, ManifestID: "m", Manifest: model.Manifest{ID: "m"}, At: time.Now().UTC()},
			model.CellChecked{Session: session, Branch: root, Cell: cell1, Source: "a", At: time.Now().UTC()},
...
// ---------------------------------------------------------------------------
// Focused LoadStaticEnv tests
// ---------------------------------------------------------------------------

// TestSQLiteStore_LoadStaticEnv covers manifest reconstruction, ordered
// source collection, head truncation, and forked-branch ancestry.
func TestSQLiteStore_LoadStaticEnv(t *testing.T) {
	ctx := context.Background()
	...
	// Helper: build a root branch with two committed cells.
	setupRoot := func(t *testing.T) *sqlitestore.Store {
		t.Helper()
		s := newTestStore(t)
		for _, f := range []model.Fact{
			model.SessionStarted{Session: session, RootBranch: root, At: time.Now().UTC()},
	...
	t.Run("head truncation stops at requested cell", func(t *testing.T) {
		s := setupRoot(t)
		// Request env only up to cell1, not cell2.
	...
	t.Run("forked branch includes ancestor cells up to fork point", func(t *testing.T) {
		s := setupRoot(t)
		// Fork at cell1 (before cell2 was committed on root).
		for _, f := range []model.Fact{
			model.BranchCreated{Session: session, Branch: fork, ParentCell: cell1, At: time.Now().UTC()},
		...
		// Expect ancestor cell1 from root, then cell3 from fork; cell2 from
		// root must NOT appear because it was committed after the fork point.
	...
	t.Run("branch exists with no committed cells returns empty sources", func(t *testing.T) {
		...
		// Use empty cell to request empty static env.
...
// TestSQLiteStore_EffectReplayPolicyPreserved confirms that the ReplayPolicy
// recorded on EffectStarted is correctly read back by LoadReplayPlan.
func TestSQLiteStore_EffectReplayPolicyPreserved(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)

	const (
		session  = model.SessionID("sess-policy")
		branch   = model.BranchID("br")
		cell     = model.CellID("c1")
		effectID = model.EffectID("fx1")
	)
	...
	for _, f := range []model.Fact{
		model.ManifestAttached{
			Session:    session,
			ManifestID: "m",
			Manifest:   model.Manifest{ID: "m"},
			At:         time.Now().UTC(),
		},
		model.CellChecked{Session: session, Branch: branch, Cell: cell, Source: "x", At: time.Now().UTC()},
		model.CellEvaluated{Session: session, Branch: branch, Cell: cell, LinkedEffects: []model.EffectID{effectID}, At: time.Now().UTC()},
		model.EffectStarted{
			Session:      session,
			Effect:       effectID,
			Cell:         cell,
		...
		model.HeadMoved{Session: session, Branch: branch, Next: cell, At: time.Now().UTC()},
	} {
		if err := s.AppendFact(ctx, f); err != nil {
			t.Fatalf("AppendFact(%q): %v", f.FactType(), err)
		}
	...
	plan, err := s.LoadReplayPlan(ctx, session, cell)
	if err != nil {
		t.Fatalf("LoadReplayPlan: %v", err)
	}
	dec, ok := plan.Decisions[effectID]
...
// ---------------------------------------------------------------------------
// LoadReplayPlan tests — branched fixture
// ---------------------------------------------------------------------------

// TestSQLiteStore_LoadReplayPlan exercises branched replay-plan construction,
// effect decisions, negative inputs, and boundary conditions.
//
// Fixture layout:
//
...
func TestSQLiteStore_LoadReplayPlan(t *testing.T) {
	...
	// buildFixture appends the complete branched session fixture to s.
	buildFixture := func(t *testing.T, s *sqlitestore.Store) {
		t.Helper()
		for _, f := range []model.Fact{
			// Session + manifest.
			model.SessionStarted{Session: session, RootBranch: root, At: time.Now().UTC()},
	...
	t.Run("root-only replay up to cell2", func(t *testing.T) {
		...
		// Expect exactly two steps: cell1 then cell2.
		if len(plan.Steps) != 2 {
			t.Fatalf("Steps len: got %d, want 2", len(plan.Steps))
		}
		...
		if step1.Source != "src2" {
			t.Errorf("Steps[1].Source: got %q, want src2", step1.Source)
		}
		if len(step1.Effects) != 1 || step1.Effects[0] != fx2 {
			t.Errorf("Steps[1].Effects: got %v, want [%q]", step1.Effects, fx2)
		}
	...
	t.Run("forked branch replay includes ancestor cells before fork cells", func(t *testing.T) {
		...
		// Steps must be: cell1 (root ancestor) THEN cell3 (fork cell).
		// cell2 must NOT appear because it is on root after the fork point.
		if len(plan.Steps) != 2 {
			t.Fatalf("Steps len: got %d, want 2 (got cells %v)", len(plan.Steps), func() []model.CellID {
		...
		// fx1 from ancestor and fx3 from fork cell must both appear.
		...
		// fx2 (root cell2, not in ancestry of fork) must be absent.
	...
	t.Run("effect without completion row yields nil RecordedResult not a panic", func(t *testing.T) {
		s := newTestStore(t)
		// Minimal fixture: one cell with one effect that is started but never completed.
		const (
...
// TestSQLiteStore_ConcurrentAppendHeadRace proves that concurrent AppendFact
// calls targeting the same session/branch do not corrupt the SQLite store.
// Multiple goroutines simultaneously append HeadMoved facts; after all writers
// finish, LoadHead must succeed and return one of the cells that was written.
//
// Run with: go test -race ./store/sqlite/...
func TestSQLiteStore_ConcurrentAppendHeadRace(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	...
	// Seed the branch with a SessionStarted fact so LoadHead can find it.
	if err := s.AppendFact(ctx, model.SessionStarted{
		Session:    session,
		RootBranch: branch,
		At:         time.Now().UTC(),
	...
	// Track which cells were written so we can verify the result.
	...
	for i, cell := range cells {
		...
		go func(idx int, c model.CellID) {
			defer wg.Done()
			// Each goroutine appends a CellChecked, CellCommitted, and HeadMoved
			// so the store has enough data for a full head query.
			for _, f := range []model.Fact{
	...
	// LoadHead must succeed and return one of the cells that was written.
	...
	// Run several concurrent reads concurrently with one final write to
	// prove WAL mode allows reader/writer coexistence.
